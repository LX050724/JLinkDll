...
int JLINKARM_ClrWP(int WPHandle) {
    return JLinkDLL_CALLPTR(P_JLINKARM_ClrWP, WPHandle);
}

int JLINKARM_ClrDataEvent(uint32_t Handle) {
    return JLinkDLL_CALLPTR(P_JLINKARM_ClrDataEvent, Handle);
}

void JLINKARM_ClrBP(unsigned int BPIndex) {
    JLinkDLL_CALLPTR(P_JLINKARM_ClrBP, BPIndex);
}

int JLINKARM_ClrBPEx(int BPHandle) {
    return JLinkDLL_CALLPTR(P_JLINKARM_ClrBPEx, BPHandle);
}

uint32_t JLINKARM_CORE_GetFound(void) {
    return JLinkDLL_CALLPTR(P_JLINKARM_CORE_GetFound);
}

void JLINKARM_EnableSoftBPs(char Enable) {
    JLinkDLL_CALLPTR(P_JLINKARM_EnableSoftBPs, Enable);
}

int JLINKARM_FindBP(uint32_t Addr) {
    return JLinkDLL_CALLPTR(P_JLINKARM_FindBP, Addr);
}

uint32_t JLINKARM_GetBPInfo(int BPHandle) {
    return JLinkDLL_CALLPTR(P_JLINKARM_GetBPInfo, BPHandle);
}

unsigned JLINKARM_GetNumBPs(void) {
    return JLinkDLL_CALLPTR(P_JLINKARM_GetNumBPs);
}

int JLINKARM_GetBPInfoEx(int iBP, JLINKARM_BP_INFO *pInfo) {
    return JLinkDLL_CALLPTR(P_JLINKARM_GetBPInfoEx, iBP, pInfo);
}

unsigned JLINKARM_GetNumWPs(void) {
    return JLinkDLL_CALLPTR(P_JLINKARM_GetNumWPs);
}

int JLINKARM_GetNumBPUnits(JLINKARM_BP_TYPE Type) {
    return JLinkDLL_CALLPTR(P_JLINKARM_GetNumBPUnits, Type);
}

int JLINKARM_GetNumWPUnits(void) {
    return JLinkDLL_CALLPTR(P_JLINKARM_GetNumWPUnits);
}

int JLINKARM_GetRegisterList(uint32_t *paList, int MaxNumItems) {
    return JLinkDLL_CALLPTR(P_JLINKARM_GetRegisterList, paList, MaxNumItems);
}

const char *JLINKARM_GetRegisterName(uint32_t RegIndex) {
    return JLinkDLL_CALLPTR(P_JLINKARM_GetRegisterName, RegIndex);
}

int JLINKARM_GetWPInfoEx(int iWP, JLINKARM_WP_INFO *pInfo) {
    return JLinkDLL_CALLPTR(P_JLINKARM_GetWPInfoEx, iWP, pInfo);
}

void JLINKARM_GoAllowSim(uint32_t NumInsts) {
    JLinkDLL_CALLPTR(P_JLINKARM_GoAllowSim, NumInsts);
}

int JLINKARM_ReadCodeMem(uint32_t Addr, uint32_t NumBytes, void *pData) {
    return JLinkDLL_CALLPTR(P_JLINKARM_ReadCodeMem, Addr, NumBytes, pData);
}

uint32_t JLINKARM_ReadICEReg(int RegIndex) {
    return JLinkDLL_CALLPTR(P_JLINKARM_ReadICEReg, RegIndex);
}

uint32_t JLINKARM_ReadReg(ARM_REG RegIndex) {
    return JLinkDLL_CALLPTR(P_JLINKARM_ReadReg, RegIndex);
}

int JLINKARM_ReadRegs(const uint32_t *paRegIndex, uint32_t *paData, uint8_t *paStatus, uint32_t NumRegs) {
    return JLinkDLL_CALLPTR(P_JLINKARM_ReadRegs, paRegIndex, paData, paStatus, NumRegs);
}

int JLINKARM_ReadTerminal(uint8_t *pBuffer, uint32_t BufferSize) {
    return JLinkDLL_CALLPTR(P_JLINKARM_ReadTerminal, pBuffer, BufferSize);
}

char JLINKARM_SimulateInstruction(uint32_t Inst) {
    return JLinkDLL_CALLPTR(P_JLINKARM_SimulateInstruction, Inst);
}

void JLINKARM_SetBP(uint32_t BPIndex, uint32_t Addr) {
    JLinkDLL_CALLPTR(P_JLINKARM_SetBP, BPIndex, Addr);
}

int JLINKARM_SetBPEx(uint32_t Addr, uint32_t TypeFlags) {
    return JLinkDLL_CALLPTR(P_JLINKARM_SetBPEx, Addr, TypeFlags);
}

int JLINKARM_SetDataEvent(JLINKARM_DATA_EVENT *pEvent, uint32_t *pHandle) {
    return JLinkDLL_CALLPTR(P_JLINKARM_SetDataEvent, pEvent, pHandle);
}

char JLINKARM_Step(void) {
    return JLinkDLL_CALLPTR(P_JLINKARM_Step);
}

char JLINKARM_StepComposite(void) {
    return JLinkDLL_CALLPTR(P_JLINKARM_StepComposite);
}

int JLINKARM_WaitForHalt(int TimeOut) {
    return JLinkDLL_CALLPTR(P_JLINKARM_WaitForHalt, TimeOut);
}

void JLINKARM_WriteICEReg(int RegIndex, uint32_t Value, int AllowDelay) {
    JLinkDLL_CALLPTR(P_JLINKARM_WriteICEReg, RegIndex, Value, AllowDelay);
}

char JLINKARM_WriteReg(ARM_REG RegIndex, uint32_t Data) {
    return JLinkDLL_CALLPTR(P_JLINKARM_WriteReg, RegIndex, Data);
}

int JLINKARM_WriteRegs(const uint32_t *paRegIndex, const uint32_t *paData, uint8_t *paStatus, uint32_t NumRegs) {
    return JLinkDLL_CALLPTR(P_JLINKARM_WriteRegs, paRegIndex, paData, paStatus, NumRegs);
}

int JLINKARM_WriteVectorCatch(uint32_t Value) {
    return JLinkDLL_CALLPTR(P_JLINKARM_WriteVectorCatch, Value);
}

int JLINKARM_ReadDCC(uint32_t *pData, uint32_t NumItems, int TimeOut) {
    return JLinkDLL_CALLPTR(P_JLINKARM_ReadDCC, pData, NumItems, TimeOut);
}

void JLINKARM_ReadDCCFast(uint32_t *pData, uint32_t NumItems) {
    JLinkDLL_CALLPTR(P_JLINKARM_ReadDCCFast, pData, NumItems);
}

int JLINKARM_WaitDCCRead(int TimeOut) {
    return JLinkDLL_CALLPTR(P_JLINKARM_WaitDCCRead, TimeOut);
}

int JLINKARM_WriteDCC(const uint32_t *pData, uint32_t NumItems, int TimeOut) {
    return JLinkDLL_CALLPTR(P_JLINKARM_WriteDCC, pData, NumItems, TimeOut);
}

void JLINKARM_WriteDCCFast(const uint32_t *pData, uint32_t NumItems) {
    JLinkDLL_CALLPTR(P_JLINKARM_WriteDCCFast, pData, NumItems);
}
//PYTHON CHECK POINT
